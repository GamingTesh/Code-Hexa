...
namespace ambiens.archtoolkit.atmaterials.utils
{
    ...
    public class Downloadcc0TexturesHandler : MonoBehaviour
    {
        ...
        public static Dictionary<RenderPipelineType, Dictionary<MaterialType, string>> materialMaps = new Dictionary<RenderPipelineType, Dictionary<MaterialType, string>>() {

            { RenderPipelineType.Builtin, new Dictionary<MaterialType, string>(){
                { MaterialType.Standard, "Standard" },
                { MaterialType.StandardSpecular, "Standard (Specular setup)" }
            } },
            { RenderPipelineType.HDRP, new Dictionary<MaterialType, string>(){
                { MaterialType.Standard, "HDRP/Lit" },
                { MaterialType.StandardSpecular, "HDRP/Lit" }
            } },
            { RenderPipelineType.URP, new Dictionary<MaterialType, string>(){
        ...
        public static void GetAndUnZip(DownloadsItem downloadsItem, string url, string relativeAssetPath, string assetPath, string name, Action<float> OnProgress, Action<string, DownloadsItem> onComplete)
        {
            ...
            tHandler.StartCoroutine(tHandler.DownloadZip(downloadsItem, url, relativeAssetPath, assetPath, name, OnProgress, onComplete));
        }

        public static void GetTextureFromSet(DownloadsItem downloadsItem, string startingUrl, List<string> set, string relativeAssetPath, string assetPath, string name, Action<float> OnProgress, Action<string, DownloadsItem> onComplete)
        {
            ...
            tHandler.StartCoroutine(tHandler.DownloadTexture(downloadsItem, startingUrl, set, relativeAssetPath, assetPath, name, OnProgress, onComplete));
        }
        ...
        private IEnumerator DownloadTexture(DownloadsItem downloadsItem, string startingUrl, List<string> set, string relativeAssetPath, string assetPath, string name, Action<float> onProgress, Action<string, DownloadsItem> onComplete)
        {
            ...
            foreach (var url in set)
            {
                ...
                using (WWW www = new WWW(remoteUrl))
                {
                    ...
                    var filepath = assetPath + Path.GetFileNameWithoutExtension(remoteUrl) + Path.GetExtension(remoteUrl);
        ...
        private IEnumerator DownloadZip(DownloadsItem downloadsItem, string url, string relativeAssetPath, string assetPath, string name, Action<float> onProgress, Action<string, DownloadsItem> onComplete)
        {
            ...
            else
            {
                ...
                using (WWW www = new WWW(url))
                {
                    ...
                    {
                        if (File.Exists(assetPath + name))
                        {
                            File.Delete(assetPath + name);
                        }
                        using (FileStream file = File.Open(assetPath + name, FileMode.CreateNew, FileAccess.Write, FileShare.None))
                        {
                            file.Write(b, 0, b.Length);
                            file.Close();
                        }
                ...
                using (UnityWebRequest www = UnityWebRequest.Get(url))
                {
                    ...
                    if (string.IsNullOrEmpty(www.error))
                    {
                        ...
                        Debug.Log("<color=red>AT+MATERIALS Error</color> Download of file "+url+" failed! => "+www.error);
                    }
        ...
        public static Material GetMaterial(string texturesPath)
        {
            ...
            Texture albedo = tList.Find(t => t.name.ContainsAny(new string[] { "col", "Albedo", "albedo", "alb", "Base_Color", "Color", "Base", "diff", "diffuse", "Diffuse" }));
            Texture normalMap = tList.Find(t => t.name.ContainsAny(new string[] { "nMap", "nrm", "NormalMap", "Normal", "normal", "nor", "nrml", "Nor" }));
            Texture occlusion = tList.Find(t => t.name.ContainsAny(new string[] { "AO", "occ", "ao", "AmbientOcclusion" }));
            Texture heightMap = tList.Find(t => t.name.ContainsAny(new string[] { "Disp", "disp", "Displacement", "heightmap", "Height", "bump", "bumpmap" }));
            Texture roughness = tList.Find(t => t.name.ContainsAny(new string[] { "roughness", "rgh", "Roughness", "rough" }));
            Texture metallic = tList.Find(t => t.name.ContainsAny(new string[] { "Metallic", "metallic", "metal", "Metalness" }));

            Texture opacity = tList.Find(t => t.name.ContainsAny(new string[] { "Opacity", "opacity" }));
            ...
            if (metallic != null)
            {
                mat = new Material(Shader.Find(materialMaps[CurrentRenderPipeline][MaterialType.Standard]));
            ...
            if (opacity != null && albedo != null)
            {
                ...
                iO.SetChannelInput(TextureChannel.ChannelRed, new TextureChannelInput(TextureChannel.ChannelAlpha, true));

                texPacker.Add(iO);

                var iA = new TextureInput();
                iA.texture = (Texture2D)albedo;
                iA.SetChannelInput(TextureChannel.ChannelRed, new TextureChannelInput(TextureChannel.ChannelRed, true));
                iA.SetChannelInput(TextureChannel.ChannelGreen, new TextureChannelInput(TextureChannel.ChannelGreen, true));
                iA.SetChannelInput(TextureChannel.ChannelBlue, new TextureChannelInput(TextureChannel.ChannelBlue, true));

                texPacker.Add(iA);

                var output = texPacker.Create();
                output.alphaIsTransparency = true;
                var maskPath = Path.Combine(Application.dataPath.Replace("Assets", "") + texturesPath, "Albedo_Opaque.png");
                ...
                var keywords = "_BLENDMODE_ALPHA _BLENDMODE_PRESERVE_SPECULAR_LIGHTING _DISPLACEMENT_LOCK_TILING_SCALE _ENABLE_FOG_ON_TRANSPARENT _HEIGHTMAP _MASKMAP _NORMALMAP _NORMALMAP_TANGENT_SPACE _SURFACE_TYPE_TRANSPARENT _VERTEX_DISPLACEMENT _VERTEX_DISPLACEMENT_LOCK_OBJECT_SCALE";
            ...
            if (CurrentRenderPipeline == RenderPipelineType.HDRP)
            {
                ...
                if (roughness != null)
                {
                    ...
                    var smoothPath = Path.Combine(Application.dataPath.Replace("Assets", "") + texturesPath, "Smoothness.png");
                    var smoothness = TextureUtility.GetSmoothnessFromRoughness(roughness as Texture2D, smoothPath);

                    var i = new TextureInput();
                    i.texture = (Texture2D)smoothness;
                    i.SetChannelInput(TextureChannel.ChannelRed, new TextureChannelInput(TextureChannel.ChannelAlpha, true));

                    texPacker.Add(i);

                    var keywords = "_DISPLACEMENT_LOCK_TILING_SCALE _HEIGHTMAP _MASKMAP _NORMALMAP _NORMALMAP_TANGENT_SPACE _VERTEX_DISPLACEMENT _VERTEX_DISPLACEMENT_LOCK_OBJECT_SCALE";
                ...
                if (metallic != null)
                {
                    ...
                    i.SetChannelInput(TextureChannel.ChannelRed, new TextureChannelInput(TextureChannel.ChannelRed, true));
                    texPacker.Add(i);
                ...
                var output = texPacker.Create();
                output.alphaIsTransparency = true;
                var maskPath = Path.Combine(Application.dataPath.Replace("Assets", "") + texturesPath, "Mask.png");
                File.WriteAllBytes(maskPath, output.EncodeToPNG());
                AssetDatabase.Refresh();
                var localPath = maskPath.Replace(Application.dataPath.Replace("Assets", ""), "");
            ...
            else if (CurrentRenderPipeline == RenderPipelineType.URP)
            {
                ...
                var output = texPacker.Create();
                output.alphaIsTransparency = true;
                var maskPath = Path.Combine(Application.dataPath.Replace("Assets", "") + texturesPath, "MetallicGloss.png");
                File.WriteAllBytes(maskPath, output.EncodeToPNG());
                AssetDatabase.Refresh();
                var localPath = maskPath.Replace(Application.dataPath.Replace("Assets", ""), "");
                var texture = AssetDatabase.LoadAssetAtPath<Texture2D>(localPath);
            ...
            if (CurrentRenderPipeline == RenderPipelineType.Builtin)
            {
                if (roughness != null)
                {
                    var smoothNessPath = Path.Combine(Application.dataPath.Replace("Assets", "") + texturesPath, "Smoothness.png");

                    var smoothness = TextureUtility.GetSmoothnessFromRoughness(roughness as Texture2D, smoothNessPath);
                    ...
                    var output = texPacker.Create();
                    output.alphaIsTransparency = true;
                    var maskPath = Path.Combine(Application.dataPath.Replace("Assets", "") + texturesPath, "Metallic.png");
                    File.WriteAllBytes(maskPath, output.EncodeToPNG());
                    AssetDatabase.Refresh();
                    var localPath = maskPath.Replace(Application.dataPath.Replace("Assets", ""), "");
        ...
        private static Texture2D SetSingleChannelSettings(Texture texture)
        {
            ...
            //s.textureType = TextureImporterType.SingleChannel;
        ...
        public static void RefreshMaterialNames()
        {
            if (GraphicsSettings.renderPipelineAsset != null)
            {
                ...
                if (GraphicsSettings.renderPipelineAsset.name.ToLower().Contains("hd"))
                {
                    ...
                    if (s == null)
                    {
                        materialMaps[CurrentRenderPipeline][MaterialType.Standard] = "HDRenderPipeline/Lit";
                        materialMaps[CurrentRenderPipeline][MaterialType.StandardSpecular] = "HDRenderPipeline/Lit";
                ...
                else if (GraphicsSettings.renderPipelineAsset.name.ToLower().Contains("lwrp"))
                {
                    ...
                    if (s == null)
                    {
                        materialMaps[CurrentRenderPipeline][MaterialType.Standard] = "Lightweight Render Pipeline/Lit";
                        materialMaps[CurrentRenderPipeline][MaterialType.StandardSpecular] = "Lightweight Render Pipeline/Lit";
                    }
                }
            }
        }
