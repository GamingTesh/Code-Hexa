    ...
    public class DownloaderTextureHandler : MonoBehaviour
    {
        ...
        private void DownloadTexture(string url, int w, int h, Action<string, Texture2D> onComplete)
        {
            ...
            else
            {
                ...
                FileAssetCache.FetchCachedAssetFromUrl(url,
                    () => { },
                    (string localCache) => {
                    ...
                    (string error) => { DestroyImmediate(this.gameObject); },
                    (float p) => { },
                    this);
       ...
       /* IEnumerator GetFromFile(string url, Action<string, Texture2D> onComplete)
        {
            Debug.Log("GetFromFile " + url);
            using (UnityWebRequest www = UnityWebRequestTexture.GetTexture(url))
            {
                ...
                if (string.IsNullOrEmpty(www.error))
                {
                    ...
                    nexTexture.LoadImage(www.downloadHandler.data);

                    if (onComplete != null)
                        onComplete(string.Empty, texture);
                }
                else
                {
