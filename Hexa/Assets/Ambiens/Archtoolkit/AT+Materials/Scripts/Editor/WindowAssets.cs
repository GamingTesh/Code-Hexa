...
namespace ambiens.archtoolkit.atmaterials.editor
{
    ...
    public class WindowAssets : EditorWindow
    {
        ...
        public static string AssetLocation = "Assets/Ambiens/Archtoolkit/AT+Materials/SourceMaterial.asset";
        ...
        private ATMatSocialReferements[] socialClasses = new ATMatSocialReferements[]
       {
            ...
            new ATMatSocialReferements ("UI/YouTube.png", "https://www.youtube.com/channel/UCkqMEDTMuARl75aCK5T5ryA", "Tutorial playlist","Tutorials"), // Youtube
            new ATMatSocialReferements ("UI/discord_logo.png", "https://discord.com/invite/Dph5fymmjY", "Discord Server","Support server"), // Discord
            ...
            new ATMatSocialReferements ("UI/Instagram.png", "https://www.instagram.com/ambiensvr/", "Instagram page","Instagram official page"), // Instagram 
            ...
            new ATMatSocialReferements ("UI/Almbiens.png", "https://www.ambiensvr.com","Our company site","AmbiensVR web page") // Ambiens site
        };
        ...
        public void DownloadJson()
        {
            ...
            {
                go.AddComponent<DownloaderJsonHandler>().Init((JsonSources jsonSources, string error) =>
                {
        ...
        private void OnGUI()
        {
            ...
            this.toolbarSelected = GUILayout.Toolbar(this.toolbarSelected,this.toolbar,GUILayout.Height(30));
            ...
            if(DownloadHDRIHavenHandler.CurrentRenderPipeline == DownloadHDRIHavenHandler.RenderPipelineType.HDRP)
            {
#if !ATM_HDRP_SUPPORT
                AddHDRPSupport();
#endif
        ...
        private void InfoStatusGUI()
        {
            ...
            GUI.DrawTexture(new Rect(0, 50, this.position.width, 120), WindowAssets.GetIcon("UI/AT.png"), ScaleMode.ScaleToFit);
            ...
            EditorGUILayout.LabelField("AT+Materials is part of the Archtoolkit family, a series of plugins made for architects and designers. Check our unity", welcomeTextStyle);
            
            if( GUILayout.Button("Assets store page (Click here)",this.GetBtnStyle()))
            {
                Help.BrowseURL("https://assetstore.unity.com/packages/tools/utilities/archtoolkit-beta-137202");
            }
                    ...
                    if (string.IsNullOrEmpty(this.search))
                    {
                        ...
                        if (this.selectedSource != null)
                        {
                            ...
                            if (!string.IsNullOrEmpty(this.selectedSource.img))
                            {
                                ...
                                {
                                    DownloaderTextureHandler.Get(this.selectedSource.img, 128, 128, (string error, Texture2D t) =>
                                    {
                                ...
                                if (this.sourceLogo != null)
                                    GUI.DrawTexture(new Rect(0, 70, this.position.width, 120), this.sourceLogo, ScaleMode.ScaleToFit);
        ...
        private void SelectSourceGUI()
        {
            ...
            if (selectedSourceIndex != -1 && (this.jsonSources.sources != null && this.jsonSources.sources.Count > 0))
            ...
            if (GUILayout.Button(GetIcon(ambiens.archtoolkit.atmaterials.utils.ArchMaterialDataPaths.REFRESH_IMAGE),GUILayout.Width(20),GUILayout.Height(20)))
            {
                this.DownloadJson();
            }
        ...
        private void SelectCategoryGUI()
        {
            ...
            var filteredCategories = this.selectedSource.categories.FindAll(c => c.materials.Count > 0);

            var arr = filteredCategories.Select(s => s.name + " (" + s.materials.Count + ")").ToArray();
            selectedCategoryIndex = EditorGUILayout.Popup(selectedCategoryIndex, arr,popupStyle);
            if (selectedCategoryIndex != -1 && (this.selectedSource.categories != null && this.selectedSource.categories.Count > 0))
           ...
           // GUIStyle btnStyle = new GUIStyle(GUI.skin.box);
            ...
            {
                if (GUILayout.Button(GetIcon(ambiens.archtoolkit.atmaterials.utils.ArchMaterialDataPaths.REDIRECT_IMAGE),GUILayout.Width(20),GUILayout.Height(20)))
                {
        ...
        private void ShowMaterialsGUI()
        {
            ...
            foreach (var mat in materials)
            {
                ...
                {
                    //Qui hai direttamente l'immagine da scaricare, non serve il bottone

                    var imageCache = this.imgTexCache[mat.img];
                    if (imageCache != null)
                    {
                        GUILayout.Box(imageCache, GUILayout.Width(80), GUILayout.Height(80));
                    }
                    else
                    {
                        GUILayout.Box(defaultIcon, GUILayout.Width(80), GUILayout.Height(80));
                    }
                ...
                else
                {
                    ...
                    GUILayout.Box(this.defaultIcon, GUILayout.Width(80), GUILayout.Height(80));
                    //

                    if (!this.currentLoadingTex.Contains(mat.img))
                    {
                        this.currentLoadingTex.Add(mat.img);
                        DownloaderTextureHandler.Get(mat.img, 512, 512, (string url, Texture2D text) =>
                        {
                ...
                for (int i = 0; i < mat.downloads.Count; i++)
                {
                    ...
                    l = dLink.type.Replace("<i class=\"fas fa-file-archive\"></i> ", "").Replace("<i class=\"fas fa-wrench\"></i> ", "");
                    ...
                    else
                    {
                        ...
                        if (GUILayout.Button(l))
                        {
                            if (alreadyDownloaded)
                            {
                                this.SelectMaterial(dLink.localPath/*"Assets" + relativePath + mat.name + ".mat"*/);
                            }
                            else
                            {
                                if (!string.IsNullOrEmpty(dLink.url))
                                {
                                    ...
                                    {
                                        Downloadcc0TexturesHandler.GetAndUnZip(dLink, dLink.url, "Assets" + relativePath,
                                        assetPath,
                                        mat.name + "_" + l,
                                        (float p) =>
                                        {
                                        ...
                                        (string url, DownloadsItem dLinked) =>
                                        {
                                            ...
                                            {
                                                dLinked.localPath = "Assets" + relativePath + mat.name + dLinked.type + ".mat";

                                                AssetDatabase.CreateAsset(dLinked.materialLoaded, dLinked.localPath);
                                    ...
                                    else if (dLink.url.Contains(".exr") || dLink.url.Contains(".hdr"))
                                    {
                                        DownloadHDRIHavenHandler.Get(dLink, dLink.url, "Assets" + relativePath,
                                            assetPath,
                                            mat.name + "_" + l,
                                            (float p) =>
                                            {
                                ...
                                else
                                {
                                    ...
                                    Downloadcc0TexturesHandler.GetTextureFromSet(dLink, startingUrl, dLink.set, "Assets" + relativePath,
                                        assetPath,
                                        mat.name + "_" + l,
                                        (float p) =>
                                        {
                ...
                if (GUILayout.Button(GetIcon( ArchMaterialDataPaths.REDIRECT_IMAGE), GUILayout.Width(30), GUILayout.Height(30)))
                {
        ...
        {
            string relativePath = "/AT+Materials/" + this.selectedSource.name + "/" + this.selectedCategory.name + "/" + mat.name + "/";
        ...
        public static Dictionary<string, Texture2D> icons_cache= new Dictionary<string, Texture2D>();
